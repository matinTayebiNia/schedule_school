...
class EditUniteRequest extends FormRequest
{
    ...
    {
        return [
            "class_id" => ["required", "numeric", Rule::unique("units", "class_id")->ignore($this->unit->class_id, "class_id")
                ->where("teacher_id", $this->input("teacher_id"))->where("lessen_id", $this->input("lessen_id"))],
            'teacher_id' => ["required", "numeric", Rule::unique("units", "teacher_id")->ignore($this->unit->teacher_id, "teacher_id")
                ->where("class_id", $this->input("class_id"))->where("lessen_id", $this->input("lessen_id"))],
            "lessen_id" => ["required", "numeric", Rule::unique("units", "lessen_id")->ignore($this->unit->lessen_id, "lessen_id")
                ->where("class_id", $this->input("class_id"))->where("teacher_id", $this->input("teacher_id"))],
            "weekday" => ["required", "string", Rule::unique("units", "weekday")->ignore($this->unit->id)
