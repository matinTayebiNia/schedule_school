        ...
        <div class="inline-bloc bg-white min-w-full shadow rounded-lg overflow-hidden">
            ...
            <div class="px-6">
                <div class="w-full rounded-lg block">
                    <form action="{{route("admin.units.update",$unit->id)}}" method="post" class=" w-full ">
                        @csrf
                        @method("PUT")
                        <div class="relative">
                            ...
                            <select class="js-example-basic-single foo w-1/2" id="school"
                                ...
                                @foreach(\App\Models\School::all() as $school)
                                    <option
                                        value="{{$school->id}}"
                                        {{$school->id==$unit->class->school->id?"selected":""}}>
                                        {{$school->name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="relative">
                            ...
                            <select class="js-example-basic-single foo w-1/2" id="teacher_id"
                                ...
                                @foreach(\App\Models\Teacher::all() as $teacher)
                                    <option
                                        value="{{$teacher->id}}"
                                        {{$teacher->id==old("teacher_id",$unit->teacher_id)?"selected":""}}>
                        ...
                        <div class="relative">
                            ...
                            <select class="js-example-basic-single foo w-1/2" id="lessen_id"
                                ...
                                @foreach(\App\Models\Lessen::all() as $lessen)
                                    <option
                                        value="{{$lessen->id}}"
                                        {{$lessen->id==old("lessen_id",$unit->lessen_id)?"selected":""}}>
                        ...
                        <div class="relative">
                            ...
                            <select class="js-example-basic-single foo w-1/2" id="weekday"
                                ...
                                @foreach(config("panel.weekday") as $key=>$weekday)
                                    <option
                                        value="{{$key}}"
                                        {{$key==old("weekday",$unit->weekday)?"selected":""}}>
                                        {{$weekday}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="relative">
                            ...
                            <x-input-dashboard :value="$unit->student_limit" name="student_limit" id="student_limit"
                        ...
                        <div class="relative">
                            ...
                            <x-input-dashboard type="time" :value="$unit->start_time" name="start_time" id="start_time">
                            </x-input-dashboard>
                        ...
                        <div class="relative">
                            ...
                            <x-input-dashboard type="time" :value="$unit->end_time" name="end_time" id="end_time">
                            </x-input-dashboard>
    ...
    @section("script")
        <script>
            ...
            function getClassOfSchool() {
                let url = "{{route("admin.unit.class",$unit->class->school->id)}}";
                ...
                fetch(url, {
                    ...
                    .then(res => {
                        ...
                        const select = {{$unit->class_id}};
                        res.data.forEach(item => {
                            let newOption = new Option(item.name, item.id, true, false)
                            if (item.id === select) {
